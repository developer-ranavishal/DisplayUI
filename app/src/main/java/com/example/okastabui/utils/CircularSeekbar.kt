...
//                pointerHaloPaint.alpha = alpha
    ...
    private fun initAttributes(attrArray: TypedArray) {
        circleXRadius = attrArray.getDimension(
            R.styleable.CircularSeekBar_cs_circle_x_radius,
            DEFAULT_CIRCLE_X_RADIUS
        )
        circleYRadius = attrArray.getDimension(
            R.styleable.CircularSeekBar_cs_circle_y_radius,
            DEFAULT_CIRCLE_Y_RADIUS
        )
        pointerStrokeWidth = attrArray.getDimension(
            R.styleable.CircularSeekBar_cs_pointer_stroke_width,
            DEFAULT_POINTER_STROKE_WIDTH
        )
        circleStrokeWidth = attrArray.getDimension(
            R.styleable.CircularSeekBar_cs_circle_stroke_width,
        ...
        pointerColor = attrArray.getColor(
            R.styleable.CircularSeekBar_cs_pointer_color,
        ...
        circleProgressColor = attrArray.getColor(
            R.styleable.CircularSeekBar_cs_circle_progress_color,
            DEFAULT_CIRCLE_PROGRESS_COLOR
        )
        circleFillColor = attrArray.getColor(
            R.styleable.CircularSeekBar_cs_circle_fill,
            DEFAULT_CIRCLE_FILL_COLOR
        )
        pointerAlphaOnTouch = attrArray.getInt(
            R.styleable.CircularSeekBar_cs_pointer_alpha_ontouch,
        ...
        pointerAngle = (360f + attrArray.getFloat(
            R.styleable.CircularSeekBar_cs_pointer_angle,
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        // Also make sure we st ill draw progress when has pointer or able to go negative
        val shouldHideProgress = hideProgressWhenEmpty && progressDegrees == 0f && !ableToGoNegative
...
//                }
    ...
    fun animatePointer(startValue: Float, endValue: Float) {
        val animator = ValueAnimator.ofFloat(startValue, endValue)

        animator.setDuration(800L) // Duration of 500ms for horizontal movement
    ...
    companion object {
        ...
        private const val DEFAULT_POINTER_STROKE_WIDTH = 0f
        private const val DEFAULT_POINTER_HALO_WIDTH = 0f
        ...
        private val DEFAULT_CIRCLE_PROGRESS_COLOR = Color.argb(255, 0, 0, 0)
        private val DEFAULT_POINTER_COLOR = Color.argb(255, 74, 138, 255)
        private val DEFAULT_POINTER_HALO_COLOR = Color.argb(255, 0, 0, 0)
        private val DEFAULT_POINTER_HALO_COLOR_ONTOUCH = Color.argb(255, 0, 0, 0)
        private const val DEFAULT_CIRCLE_FILL_COLOR = Color.TRANSPARENT
        private const val DEFAULT_POINTER_ALPHA = 255
        private const val DEFAULT_POINTER_ALPHA_ONTOUCH = 2
